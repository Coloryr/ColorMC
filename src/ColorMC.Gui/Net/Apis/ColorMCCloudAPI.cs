...
public static class ColorMCCloudAPI
{
    ...
    /// 云同步信息
    /// </summary>
    public static string Info { get; private set; } = LangUtils.Get("App.Text96");
    /// <summary>
    /// 是否已连接云同步服务器
    /// </summary>
    ...
    public static async Task<Dictionary<string, FrpDownloadObj>?> GetFrpListAsync()
    {
        ...
        {
            Logs.Error(LangUtils.Get("App.Error.Log1"), e);
            return null;
        }
    }
    ...
    public static async Task<JsonDocument?> GetCloudServerAsync(string version)
    {
        ...
        {
            Logs.Error(LangUtils.Get("App.Error.Log3"), e);
            return null;
        }
    }
    ...
    public static async Task<bool> PutCloudServerAsync(string token, string ip, FrpShareModel model)
    {
        ...
        {
            Logs.Error(LangUtils.Get("App.Error.Log2"), e);
        }
        return false;
    }
    ...
    public static async Task CheckAsync()
    {
        ...
        if (res.IsSuccessStatusCode)
        {
            ...
            if (json.TryGetProperty("res", out var res1) && res1.ValueKind is JsonValueKind.Number)
            {
                ...
                if (value == 300)
                {
                    Info = LangUtils.Get("App.Text98");
                    return;
                }
                else if (value != 100)
                {
                    Info = LangUtils.Get("App.Text99");
                    return;
                }
            }
            Connect = true;
    ...
    public static async Task<CloudRes> HaveCloudAsync(GameSettingObj obj)
    {
        ...
        {
            string temp = LangUtils.Get("GameCloudWindow.Text21");
            Logs.Error(temp, e);
        }
        ...
        try
        {
            ...
            if (res.IsSuccessStatusCode)
            {
                ...
                if (json.TryGetProperty("res", out var res1) && res1.ValueKind is JsonValueKind.Number)
                {
                    ...
                    if (value == 100)
                    {
                        Info = string.Format(LangUtils.Get("App.Text94"),
