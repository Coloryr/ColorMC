...
/// 用户子项目
/// </summary>
public partial class UserItemGridControl : UserControl
{
    private Point _last;
    private bool _press;
    private double _now;
    ...
    public UserItemGridControl()
    {
        InitializeComponent();

        DoubleTapped += UserItemControl_DoubleTapped;
        PointerPressed += UserItemControl_PointerPressed;
        PointerEntered += UserItemGridControl_PointerEntered;
        PointerExited += UserItemGridControl_PointerExited;

        Border1.PointerPressed += Border1_PointerPressed;
        Border1.PointerReleased += Border1_PointerReleased;
    }

    private void UserItemGridControl_PointerExited(object? sender, PointerEventArgs e)
    {
    ...
    private void UserItemGridControl_PointerEntered(object? sender, PointerEventArgs e)
    {
        if (DataContext is UserDisplayModel model)
        {
            model.Top = true;
